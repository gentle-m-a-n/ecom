...
export const createCheckoutSession = async (req, res) => {
 try {
    ...
    for (const product of product) {
        ...
        if (checkProduct.stockQuantity < product.quantity) {
            return res.status(400).json({
                message: `Not enough stock for product: ${checkProduct.name}`
            });
        }
    } 
...
return coupon.id}
 ...
 async function createNewCoupon(userId){
    const newCoupon = new Coupon({
        code: "DISCOUNT" + Math.random().toString(36).substring(2, 8),
